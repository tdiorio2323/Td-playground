...
export default function Breakout({ w = 300, h = 200 }: { w?: number; h?: number }) {
  ...
  const [state, setState] = useState<"waiting" | "countdown" | "playing" | "gameOver" | "won">(
    "waiting",
  );
  ...
  useEffect(() => {
    ...
    const paddle = { x: w / 2 - 25, w: 50, h: 8 },
      ball = { x: w / 2, y: h - 50, dx: 3, dy: -3, r: 4 };
    ...
    for (let r = 0; r < rows; r++)
      for (let cix = 0; cix < cols; cix++)
        bricks.push({ x: cix * (bw + 2) + 1, y: r * (bh + 2) + 30, w: bw, h: bh, hit: false });
    ...
    const draw = () => {
      ...
      for (const b of bricks) {
        if (!b.hit) {
          ctx.fillStyle = `hsl(${((b.x + b.y) * 2) % 360},70%,60%)`;
      ...
      if (state === "waiting") {
        ...
        ctx.fillText("Breakout Game", w / 2, h / 2 + 10);
      }
      if (state === "countdown") {
        ...
        ctx.fillText("Touch & drag paddle to move", w / 2, h / 2 + 40);
      }
    ...
    const tm = (e: TouchEvent) => {
      if (state !== "countdown" && !playing.current) return;
      e.preventDefault();
    ...
    const ts = () => {
      if (state === "waiting" || state === "gameOver" || state === "won") start();
    };
    ...
    return () => {
      ...
      c.removeEventListener("click", ts);
    };
  }, [w, h, state, countdown, score]);

  return (
    <canvas ref={canvasRef} width={w} height={h} className="block touch-none cursor-pointer" />
  );
}

      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-12">
        <header className="space-y-3">
          ...
          <p className="text-muted-foreground">
            Preview low-resolution thumbnails of our cannabis bag label collection. Full-resolution artwork is
          ...
          <Alert className="border border-primary/30 bg-primary/5">
            ...
            <AlertDescription>
              These previews are intentionally compressed and protected from right-click save actions. The delivered
              files include print-ready vector exports.
            </AlertDescription>
          </Alert>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {designs.map((design, index) => (
              <Card key={design.id} className="overflow-hidden border border-border/60 bg-card/80 backdrop-blur">
                <CardContent className="p-0">
                  ...
                  <CardHeader className="space-y-2">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-muted-foreground">
                      ...
                      {design.updatedAt && <span>{new Date(design.updatedAt).toLocaleDateString()}</span>}
                    </div>
                    {import.meta.env.DEV && (
        ...
        {!isLoading && designs.length === 0 && (
          <Alert>
            ...
            <AlertDescription>
              Upload artwork to the `bag-designs/public` folder and refresh this page to see previews.
            </AlertDescription>
          </Alert>
        )}
      </div>
